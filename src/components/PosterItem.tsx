...
function PosterItem({ poster }: Prop) {
    ...
    if (typeOfJob === "task") {
        return (
            <div
                style={{ borderBottom: "1px solid #7E7E7E" }}
                id="poster-item"
                className="flex flex-col justify-evenly my-3 h-68 pb-5 "
            >
                <h3 className="m-0">{title}</h3>
                <div>
                    <p className="m-0 text-sm">{`Owner: &ensp; ${applicants}`}</p>
                ...
                <div id="tag">
                    {skills.map((skill) => (
                        <button
                            style={{ color: "#606060" }}
                            className="px-5 py-2 rounded-2xl border-none cursor-pointer"
                            type="button"
                        >
                            {skill}
                        </button>
